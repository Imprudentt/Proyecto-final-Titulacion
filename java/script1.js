function updatePrice(asesorId) {
    const modeSelect = document.getElementById(`${asesorId}-mode`);
    const citySelect = document.getElementById(`${asesorId}-city`);
    const locationDiv = document.getElementById(`${asesorId}-location`);
    const priceTag = document.getElementById(`${asesorId}-price`);
    
    if (modeSelect.value === 'presencial') {
        locationDiv.style.display = 'block';
        switch (citySelect.value) {
            case 'santiago':
                priceTag.textContent = 'Precio: $80.000 CLP';
                break;
            case 'valparaiso':
                priceTag.textContent = 'Precio: $70.000 CLP';
                break;
            case 'concepcion':
                priceTag.textContent = 'Precio: $75.000 CLP';
                break;
            default:
                priceTag.textContent = 'Precio: $80.000 CLP';
                break;
        ...
        priceTag.textContent = 'Precio: $50.000 CLP';
    }
}
...
document.querySelectorAll('.toggle-form').forEach(button => {
    button.addEventListener('click', function() {
        const form = this.nextElementSibling;  // Selecciona el div de detalles solo en el mismo card
        const allForms = document.querySelectorAll('.advisor-details');

        // Oculta todos los forms antes de mostrar el actual
        allForms.forEach(f => {
            if (f !== form) {
